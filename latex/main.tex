...
\subsection{级数截断 (Truncation)}
为了数值求解，我们将无限求和 $\sum_{n=-\infty}^{\infty}$ 截断为 $\sum_{n=-N}^{N}$。
\begin{itemize}
    \item 选取 $N=1$（即 $n=-1, 0, 1$）。
    \item 未知数总数变为：5 (反射系数) + $5 \times (2N+1)$ (散射系数) = 20 个。
    \item 矩阵 $\mathbf{M}$ 的尺寸相应扩展为 $20 \times 20$。
\end{itemize}

\subsection{离散波数法 (Discrete Wavenumber Method)}
为了计算积分项 $\mathbf{M}_{FC}$（Sommerfeld 积分），我们采用离散波数求和：
\begin{equation}
    \Lambda(h) \approx \sum_{k_x} e^{-\gamma h} \cdot R(k_x) \cdot \frac{1}{\gamma} \Delta k_x
\end{equation}
该方法有效地避开了 Rayleigh 极点的积分发散问题，将连续谱转化为有限的平面波叠加。
...
\begin{itemize}
    \item 求解耗时：$< 1$ 秒。
    \item 矩阵条件数：良好 (非奇异)。
    \item 典型结果：反射波系数主幅值 $|A_{R1}| \approx 1.93 \times 10^8$（响应强烈）。
\end{itemize}
